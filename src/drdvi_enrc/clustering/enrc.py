    ...
    def fit(self, trainloader: torch.utils.data.DataLoader, evalloader: torch.utils.data.DataLoader,
        ...
        for epoch_i in tbar:
            ...
            for batch in trainloader:
                ...
                if hasattr(model, "enrc_loss"):
                    ssl_loss = model.enrc_loss(batch_data)
                else:
                ...
                if self.augmentation_invariance:
                    ...
                    if hasattr(model, "enrc_loss"):
                        ssl_loss_aug = model.enrc_loss(batch_data_aug)
                    else:
                ...
                if debug:
                    batch_loss_components = {
                        "cluster_loss": subspace_loss,
                        "ssl_loss": ssl_loss,
                        "weighted_cluster_loss": self.clustering_loss_weight * subspace_loss,
                        "weighted_ssl_loss": self.ssl_loss_weight * ssl_loss,
                    }
                    if hasattr(model, "last_loss_components"):
                        batch_loss_components.update(model.last_loss_components)
                    for name, value in batch_loss_components.items():
                        if torch.is_tensor(value):
                            value = value.detach().cpu().item()
                        epoch_loss_components[name] = epoch_loss_components.get(name, 0.0) + float(value)
                optimizer.zero_grad()
            ...
            history_row = {"epoch": epoch_i, "total_loss": total_loss}
            if debug and n_batches > 0:
                for name, value in epoch_loss_components.items():
                    mean_value = value / n_batches
                    postfix_str[name] = mean_value
                    history_row[name] = mean_value
            with torch.no_grad():
                ...
                history_row["rotation_loss"] = rotation_loss.item()
            tbar.set_postfix(postfix_str)
            self.loss_history_.append(history_row)
...
def _enrc(X: np.ndarray, n_clusters: list, V: np.ndarray, P: list, input_centers: list, batch_size: int,
    ...
    enrc_module.fit(trainloader=trainloader,
                    ...
                    debug=debug)
    neural_network.enrc_loss_history_ = enrc_module.loss_history_
